...
<%@page import="com.Negocio.PlantillaSMS"%>
...
<link href="estilos/estilos.css" rel="stylesheet" type="text/css" />
 ...
 <script type="text/javascript">
    ...
    function validar()
    {
        ...
        if (nombre.length == 0)
        {
            alert("Debes introducir el nombre de la Plantilla SMS");
            return false;
        }
        else
        {
            if (texto.length == 0)
            {
                alert("Debes introducir el texto de la Plantilla SMS");
                return false;
            }
            else
            {
    ...
    {
        idPlantillaSMS = document.getElementById("idPlantillaSMS").value;
        if (idPlantillaSMS.length == 0 || idPlantillaSMS == "/")
        {
...
<div id="contenedor">
	...
	<div id="menu">
		...
		<li><a href="http://www.bizkaiagolf.com">WEB</a></li>
         ...
         <div id="submenu">
            <ul id="lista_submenu">
                        ...
                        <li><a id="activo" href="#">Plantilla SMS</a></li>
        ...
        <%
            ArrayList<PlantillaSMS> listaPlantillaSMS = (ArrayList<PlantillaSMS>)session.getAttribute("listaPlantillaSMS");
        %>

 	<div id="principal">

	<h1><img src="imagenes/carpeta.gif" width="30" height="30" /> CONFIGURACI&Oacute;N PLANTILLA SMS</h1>

      <div class="marco">
  		<div class="msg_head_gris">
    	<img src="imagenes/lapiz.gif" width="16" height="16" />Tipos SMS</div>
	...
	<tr>
            <th class="titulos_tablas">Nombre Plantilla SMS</th>
            <th class="titulos_tablas">Texto</th>
        </tr>
	</thead>

	<tbody>

            <% if (listaPlantillaSMS != null)
               {
                    for (int i = 0; i<listaPlantillaSMS.size(); i++)
                    {
            %>
            <tr>
                <td class="texto_tablas"><a href="confPlantillaSMSServlet?var=<%=listaPlantillaSMS.get(i).getIdPlantillaSMS()%>"><%=listaPlantillaSMS.get(i).getNombre()%></a></td>
                <td class="texto_tablas"><a href="confPlantillaSMSServlet?var=<%=listaPlantillaSMS.get(i).getIdPlantillaSMS()%>"><%=listaPlantillaSMS.get(i).getTexto()%></a></td>
            </tr>
        ...
        <%
        PlantillaSMS plantillaSMS = (PlantillaSMS)session.getAttribute("unaPlantillaSMS");
        String idPlantillaSMS = "";
        String nombre = "";
        String texto = "";
        if (plantillaSMS != null)
        {
            idPlantillaSMS = String.valueOf(plantillaSMS.getIdPlantillaSMS());
            nombre = plantillaSMS.getNombre();
            texto = plantillaSMS.getTexto();
        }
     %>

        <form autocomplete="off" action="confPlantillaSMSMIBServlet" method="post">
    ...
    <div class="formulario_formato">
	<div class="formulario_dobles">Texto:</div>
	<div class="formulario_dobles_campos">
	<label>
            <textarea name="texto" cols="100" rows="2" class="campo_texto" id="texto"><%=texto%></textarea>
	</label></div>
	</div>
        
            <input type="hidden" id="idPlantillaSMS"  name="idPlantillaSMS" value=<%=idPlantillaSMS%> /><br/>
